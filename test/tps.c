...
void *thread2(void* arg)
{
	...
	char* buffer2 = malloc(TPS_SIZE);
	/* Create TPS and initialize with *msg1 */
	//tps_create();
	assert(tps_create() != -1); //returns -1 when it fails
	...
	/* Write into TPS and read out and make sure it is the same */
	tps_write(0, TPS_SIZE, msg1); 
	tps_read(0, TPS_SIZE, buffer2);
	assert(strcmp(msg1, buffer2) == 0);

	/* Test out of bounds check for write */
	assert(tps_write(TPS_SIZE, TPS_SIZE, msg1) == -1);
	assert(tps_write(2, TPS_SIZE, msg1) == -1);
